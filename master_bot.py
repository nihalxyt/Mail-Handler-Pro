...
# =====================================================================
# SMTP SERVER CONFIG (replaces IMAP polling)
# =====================================================================
...
# =====================================================================
mongo1   = AsyncIOMotorClient(BOT1_MONGO_URI)
...
mongo2   = AsyncIOMotorClient(BOT2_MONGO_URI)
...
# admin_state per bot  {sender_id: state_dict}
bot1_admin_state: Dict[int, dict] = {}
bot2_admin_state: Dict[int, dict] = {}


# =====================================================================
# HELPER: resolve bot context by instance
# =====================================================================
def _ctx(bot_instance):
    """Return (col_users, col_aliases, col_logs, alias_cache, alias_token_cache,
               admin_state, super_admin_ids, bot_label) for the given bot."""
    if bot_instance is bot1:
        return (bot1_col_users, bot1_col_aliases, bot1_col_logs,
                bot1_alias_cache, bot1_alias_token_cache,
                bot1_admin_state, BOT1_SUPER_ADMIN_IDS, "Bot1")
    else:
        return (bot2_col_users, bot2_col_aliases, bot2_col_logs,
                bot2_alias_cache, bot2_alias_token_cache,
                bot2_admin_state, BOT2_SUPER_ADMIN_IDS, "Bot2")
...
def extract_original_recipient(msg, gmail_user=""):
    priority_headers = [
        "X-Original-To", "X-Gm-Original-To", "X-Forwarded-To",
        "Delivered-To", "Envelope-To", "X-Envelope-To",
    ]
    for h in priority_headers:
        val = msg.get(h)
        if val:
            val = val.strip().lower()
            if "@" in val and val != gmail_user.lower():
                return val
    return None

def extract_all_recipients(msg, gmail_user=""):
    headers_to_check = [
        "X-Original-To", "X-Gm-Original-To", "X-Forwarded-To",
        "Delivered-To", "Envelope-To", "X-Envelope-To",
        "To", "Cc", "Bcc",
    ...
    for h in headers_to_check:
        ...
        if isinstance(values, list):
            for v in values:
                for _name, addr in getaddresses([str(v)]):
                    if addr:
                        found.add(addr.lower())
        else:
            for _name, addr in getaddresses([str(values)]):
                if addr:
                    found.add(addr.lower())
    if not found:
        raw_headers = ""
        for key in msg.keys():
            raw_headers += f"{key}: {msg[key]}\n"
        for m in EMAIL_RE.findall(raw_headers):
            found.add(m.lower())
    if gmail_user:
        found.discard(gmail_user.lower())
    return found
...
def clean_html_body(html_text, max_len=4000):
    ...
    text = re.sub(r'<(script|style|head|title|meta)[^>]*>.*?</\1>', '', text, flags=re.DOTALL | re.IGNORECASE)
    ...
    text = re.sub(r'<(code|pre)[^>]*>(.*?)</\1>', r'\2', text, flags=re.DOTALL | re.IGNORECASE)
    text = re.sub(r'</?(div|p|h[1-6]|tr|li|ul|ol|blockquote|table)[^>]*>', '\n', text, flags=re.IGNORECASE)
    text = re.sub(r'<br\s*/?>', '\n', text, flags=re.IGNORECASE)
    ...
    text = re.sub(r'<a\s+(?:[^>]*?\s+)?href=["\'](.*?)["\'][^>]*>(.*?)</a>', link_repl, text, flags=re.DOTALL | re.IGNORECASE)
    text = re.sub(r'<[^>]+>', '', text)
...
def get_first_super_admin(super_admin_ids):
    if super_admin_ids:
        return next(iter(super_admin_ids))
    return None
...
async def refresh_alias_cache(col_aliases, alias_cache, force=False):
    ...
    all_aliases_cur = col_aliases.find({}, {"alias_email": 1})
    all_emails = set()
    async for a in all_aliases_cur:
...
async def refresh_alias_tokens(col_aliases, alias_token_cache_dict, force=False):
    cur = col_aliases.find({})
    async for a in cur:
        token = sha256(a["alias_email"])[:12]
        alias_token_cache_dict[token] = a["alias_email"].lower()
...
async def refresh_bot1_alias_tokens(force=False):
    ...
    if not force and (time.time() - bot1_alias_token_updated_at < 30):
        return
    bot1_alias_token_cache.clear()
    await refresh_alias_tokens(bot1_col_aliases, bot1_alias_token_cache, force)
...
async def refresh_bot2_alias_tokens(force=False):
    ...
    if not force and (time.time() - bot2_alias_token_updated_at < 30):
        return
    bot2_alias_token_cache.clear()
    await refresh_alias_tokens(bot2_col_aliases, bot2_alias_token_cache, force)
    bot2_alias_token_updated_at = time.time()


# =====================================================================
# USER / ACCESS HELPERS  (generic — pass col_users + super_admin_ids)
# =====================================================================
async def is_super_admin(tg_user_id, super_admin_ids):
    return tg_user_id in super_admin_ids

async def is_admin(tg_user_id, col_users, super_admin_ids):
...
async def get_or_create_user(tg_user_id, col_users, super_admin_ids, username="", name=""):
    u = await col_users.find_one({"tg_user_id": tg_user_id})
    if u:
...
async def check_user_access(tg_user_id, col_users):
    u = await col_users.find_one({"tg_user_id": tg_user_id})
    if not u:
...
# =====================================================================
# KEYBOARD BUILDERS  (shared — identical for both bots)
# =====================================================================
def admin_main_kb():
    return [
...
# =====================================================================
# PANEL / VIEW FUNCTIONS  (generic — pass bot + col references)
# =====================================================================
async def admin_panel(event, bot_instance, col_users, col_aliases, col_logs, edit=False):
    total_users   = await col_users.count_documents({})
    active_users  = await col_users.count_documents({"status": "active"})
    pending_users = await col_users.count_documents({"status": "pending"})
    banned_users  = await col_users.count_documents({"status": "banned"})
    total_aliases   = await col_aliases.count_documents({"active": True})
    expired_aliases = await col_aliases.count_documents({"$or": [{"active": False}, {"expires_at": {"$lte": now_utc()}}]})
    today_mails = await col_logs.count_documents({"received_at": {"$gte": day_start_utc()}})
    total_mails = await col_logs.count_documents({})
    text = (
        "🛠️ **Admin Dashboard**\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "👥 **Users:**\n"
...
async def user_panel(event, col_users, col_aliases, col_logs, edit=False):
    u = await col_users.find_one({"tg_user_id": event.sender_id})
    if not u:
        return await event.respond("❌ User not found. Please use /start")
    active_aliases = await col_aliases.count_documents({
        "tg_user_id": event.sender_id, "active": True, "expires_at": {"$gt": now_utc()}
    })
    unread_mails = await col_logs.count_documents({
        "tg_user_id": event.sender_id, "deleted": {"$ne": True}, "read": {"$ne": True}
    })
    starred_mails = await col_logs.count_documents({
...
async def show_starred(event, col_logs, page=0, edit=False):
    sender_id = event.sender_id
    logs = await col_logs.find({
        "tg_user_id": sender_id, "deleted": {"$ne": True}, "starred": True
    }).sort("received_at", -1).skip(page * PAGE_SIZE).limit(PAGE_SIZE + 1).to_list(PAGE_SIZE + 1)
    has_more = len(logs) > PAGE_SIZE
    logs = logs[:PAGE_SIZE]
...
async def show_user_stats(event, col_users, col_aliases, col_logs, edit=False):
    u = await col_users.find_one({"tg_user_id": event.sender_id})
    total_aliases  = await col_aliases.count_documents({"tg_user_id": event.sender_id})
    active_aliases = await col_aliases.count_documents({"tg_user_id": event.sender_id, "active": True, "expires_at": {"$gt": now_utc()}})
    total_mails    = await col_logs.count_documents({"tg_user_id": event.sender_id, "deleted": {"$ne": True}})
    unread_mails   = await col_logs.count_documents({"tg_user_id": event.sender_id, "read": {"$ne": True}, "deleted": {"$ne": True}})
    starred        = await col_logs.count_documents({"tg_user_id": event.sender_id, "starred": True, "deleted": {"$ne": True}})
    today_mails    = await col_logs.count_documents({"tg_user_id": event.sender_id, "received_at": {"$gte": day_start_utc()}})
    text = (
        "📊 Your Statistics\n━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📅 Account:\n"
        f" └ Member Since: {format_datetime(u.get('created_at'))}\n\n"
        f"📧 Email Aliases:\n"
...
async def show_user_settings(event, col_users, col_logs, edit=False):
    u = await col_users.find_one({"tg_user_id": event.sender_id})
...
async def show_admin_stats(event, col_users, col_aliases, col_logs):
    total_users    = await col_users.count_documents({})
    active_users   = await col_users.count_documents({"status": "active"})
    pending        = await col_users.count_documents({"status": "pending"})
    banned         = await col_users.count_documents({"status": "banned"})
    total_aliases  = await col_aliases.count_documents({})
    active_aliases = await col_aliases.count_documents({"active": True, "expires_at": {"$gt": now_utc()}})
    total_mails    = await col_logs.count_documents({})
    today_mails    = await col_logs.count_documents({"received_at": {"$gte": day_start_utc()}})
    deleted_mails  = await col_logs.count_documents({"deleted": True})
    unread_mails   = await col_logs.count_documents({"read": {"$ne": True}, "deleted": {"$ne": True}})
    text = (
        "📊 **System Statistics**\n━━━━━━━━━━━━━━━━━━━━\n\n"
        "👥 **Users:**\n"
        f" ├ Total: {total_users}\n"
...
async def show_user_list(event, col_users, col_aliases, status_filter, page=0):
    ...
    text = (
        f"👥 **{title} Users** — Page {page + 1}/{total_pages}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"Total: {total}\n\n"
    )
    buttons = []
...
async def show_user_detail(event, tg_id, col_users, col_aliases, col_logs):
    u = await col_users.find_one({"tg_user_id": tg_id})
    if not u:
...
async def show_user_emails_admin(event, tg_id, col_users, col_aliases, col_logs):
    now = now_utc()
...
async def show_alias_list(event, col_users, col_aliases, list_type, page=0):
    now = now_utc()
    if list_type == "active":
    ...
    text = (
        f"📧 **{title} Emails** — Page {page + 1}/{total_pages}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"Total: {total}\n\n"
    )
    buttons = []
    for a in aliases:
        ...
        status_emoji = "✅" if is_active else "❌"
        token = sha256(a["alias_email"])[:12]
        u = await col_users.find_one({"tg_user_id": a.get("tg_user_id")})
        uname = u.get("name", "?") if u else "?"
        text += f"{status_emoji} `{a['alias_email']}`\n"
        text += f" ├ User: {short(uname, 15)} (`{a.get('tg_user_id')}`)\n"
        text += f" └ Expires: {time_remaining(exp_aware)}\n\n"
        buttons.append([Button.inline(f"{status_emoji} {short(a['alias_email'], 30)}", cb("EA", token, "view"))])
    if not aliases:
        text += "_No emails found._"
...
async def show_alias_detail(event, alias_email, col_users, col_aliases, col_logs):
    ...
    u = await col_users.find_one({"tg_user_id": alias.get("tg_user_id")})
    uname = u.get("name", "Unknown") if u else "Unknown"
...
async def show_admin_user_stats(event, tg_id, col_users, col_aliases, col_logs):
    u = await col_users.find_one({"tg_user_id": tg_id})
    if not u:
        return await event.edit("❌ User not found.", buttons=[[Button.inline("⬅️ Back", cb("A", "users"))]])
    total_aliases  = await col_aliases.count_documents({"tg_user_id": tg_id})
...
async def show_admin_user_inbox(event, tg_id, col_users, col_logs, page=0):
    logs = await col_logs.find({
...
async def show_user_overview(event, col_users):
    total   = await col_users.count_documents({})
    active  = await col_users.count_documents({"status": "active"})
    pending = await col_users.count_documents({"status": "pending"})
    banned  = await col_users.count_documents({"status": "banned"})
    admins  = await col_users.count_documents({"role": {"$in": ["admin", "super_admin"]}})
    text = (
        "📊 **User Overview**\n━━━━━━━━━━━━━━━━━━━━\n\n"
        f"👥 Total: **{total}**\n"
        f"✅ Active: **{active}**\n"
...
async def show_email_overview(event, col_aliases):
    now    = now_utc()
    total  = await col_aliases.count_documents({})
    active = await col_aliases.count_documents({"active": True, "expires_at": {"$gt": now}})
    expired = await col_aliases.count_documents({"$or": [{"active": False}, {"expires_at": {"$lte": now}}]})
...
async def show_activity_log(event, col_users, col_logs):
    ...
    for lg in recent_mails:
        ...
        text += f"   {format_datetime(lg.get('received_at'))}\n"
    return await event.edit(text, buttons=[[Button.inline("⬅️ Back", cb("A", "back"))]])
...
async def finalize_add_email(admin_tg_id, event, days, bot_instance, col_aliases, alias_cache, alias_token_cache_dict, admin_state):
...
# =====================================================================
# PROCESS EMAIL  (called by SMTP handler)
# =====================================================================
async def process_incoming_email(rcpt_to: str, raw_data: bytes):
    """Route incoming SMTP email to the correct bot and user."""
    try:
        msg = email.message_from_bytes(raw_data)
        to_addr = rcpt_to.lower().strip()

        # Try Bot1 first, then Bot2
        for (
            bot_instance,
            col_aliases,
            col_users,
            ...
            (bot1, bot1_col_aliases, bot1_col_users, bot1_col_logs, bot1_alias_cache, BOT1_SUPER_ADMIN_IDS, "Bot1"),
            (bot2, bot2_col_aliases, bot2_col_users, bot2_col_logs, bot2_alias_cache, BOT2_SUPER_ADMIN_IDS, "Bot2"),
        ]:
            if to_addr in alias_cache["by_email"]:
                await _deliver_email(bot_instance, col_aliases, col_users, col_logs,
                                     alias_cache, super_admin_ids, bot_label,
                                     msg, raw_data, to_addr)
                return

        # Not found in either cache — fallback to bot1 admin
        logger.warning(f"[SMTP] No alias found for {to_addr}, routing to admin fallback (Bot1)")
...
async def _deliver_email(bot_instance, col_aliases, col_users, col_logs,
    ...
    user = await col_users.find_one({"tg_user_id": tg_user_id})
    ...
    log_doc = {
        ...
        "alias_email": matched_alias,
        "original_to": matched_alias,
        "tg_user_id": tg_user_id,
        "from": sender,
        "subject": subject,
        "date_header": date_hdr,
        "received_at": now_utc(),
        "snippet": short(clean_prev, 220),
        "body": body,
        "read": False,
        "deleted": False,
        "starred": False,
...
async def _admin_fallback_email(bot_instance, col_logs, super_admin_ids,
    ...
    admin_tg = get_first_super_admin(super_admin_ids)
    if not admin_tg:
        return
    if sender is None:
        sender = decode_str(msg.get("From", ""))
    ...
    if dedupe_key is None:
        msg_id = decode_str(msg.get("Message-ID", "")) or ""
        dedupe_key = sha256(f"{msg_id}|{alias_email}|{subject}|{sender}|fallback")
    ...
    log_doc = {
        ...
        "received_at": now_utc(),
        "snippet": short(clean_preview, 220),
        "body": body,
        "read": False,
        "deleted": False,
        "starred": False,
...
# =====================================================================
# GENERIC CALLBACK HANDLER FACTORY
# =====================================================================
def make_callback_handler(
    bot_instance,
    col_users, col_aliases, col_logs,
    ...
    async def on_callback(event):
        ...
        async def _is_admin():
            return await is_admin(event.sender_id, col_users, super_admin_ids)

        async def _check_access():
            return await check_user_access(event.sender_id, col_users)
        ...
        if parts[0] == "CS":
            ...
            if has_access:
                ...
                return await user_panel(event, col_users, col_aliases, col_logs, edit=True)
        ...
        if parts[0] == "X":
            ...
            if await _is_admin():
                return await admin_panel(event, bot_instance, col_users, col_aliases, col_logs, edit=True)
            return await user_panel(event, col_users, col_aliases, col_logs, edit=True)
        ...
        if parts[0] == "A":
            ...
            if action == "back":
                ...
                return await show_admin_stats(event, col_users, col_aliases, col_logs)
        ...
        if parts[0] == "U":
            ...
            if action == "add":
                ...
                return await show_user_overview(event, col_users)
            return

        if parts[0] == "UL":
            if not await _is_admin():
                return await event.answer("❌ Admin only", alert=True)
            status_filter = parts[1] if len(parts) > 1 else "active"
            page = int(parts[2]) if len(parts) > 2 else 0
            return await show_user_list(event, col_users, col_aliases, status_filter, page)
        ...
        if parts[0] == "UM":
            ...
            if action == "view":
                return await show_user_detail(event, tg_id, col_users, col_aliases, col_logs)
            elif action == "approve":
                ...
                target = await col_users.find_one({"tg_user_id": tg_id})
                ...
                return await show_user_emails_admin(event, tg_id, col_users, col_aliases, col_logs)
                ...
                return await show_admin_user_stats(event, tg_id, col_users, col_aliases, col_logs)
            elif action == "inbox":
                page = int(parts[3]) if len(parts) > 3 else 0
                return await show_admin_user_inbox(event, tg_id, col_users, col_logs, page)
        ...
        if parts[0] == "E":
            ...
            if action == "add":
                ...
                page      = int(parts[3]) if len(parts) > 3 else 0
                return await show_alias_list(event, col_users, col_aliases, list_type, page)
            elif action == "overview":
                return await show_email_overview(event, col_aliases)
            elif action == "cleanup":
        ...
        if parts[0] == "EA":
            ...
            if action_val == "view":
                ...
                return await show_alias_detail(event, alias_email, col_users, col_aliases, col_logs)
        ...
        if parts[0] == "ED":
            ...
            if not alias_email:
                return await event.answer("❌ Email not found", alert=True)
            await col_aliases.update_one({"alias_email": alias_email}, {"$set": {"active": False, "updated_at": now_utc()}})
            await refresh_cache_fn(force=True)
        ...
        if parts[0] == "EP":
            ...
            if not alias_email:
                return await event.answer("❌ Email not found", alert=True)
            await col_aliases.delete_one({"alias_email": alias_email})
            await refresh_cache_fn(force=True)
            await refresh_tokens_fn(force=True)
        ...
        if parts[0] == "D":
            ...
            if val.isdigit():
                return await finalize_add_email(
                    ...
                    bot_instance, col_aliases, alias_cache, alias_token_cache_dict, admin_state
                )
            return
        ...
        if parts[0] == "M":
            ...
            if action == "back":
                ...
                return await show_user_stats(event, col_users, col_aliases, col_logs, edit=True)
            elif action == "settings":
                return await show_user_settings(event, col_users, col_logs, edit=True)
            elif action == "help":
        ...
        if parts[0] == "ML":
            ...
            lg = await col_logs.find_one({
                "_id": {"$regex": f"^{re.escape(log_id_prefix)}"},
            ...
            from_str    = lg.get('from', 'Unknown')
            subject_str = lg.get('subject', 'No Subject')
            original_to = lg.get('original_to', lg.get('alias_email', ''))
            star_icon   = "⭐ " if lg.get("starred") else ""
            text = (
                f"{star_icon}📨 **Mail Reader**\n"
                "━━━━━━━━━━━━━━━━━━━━\n\n"
                f"👤 **From:** `{short(from_str, 50)}`\n"
                f"🎯 **To:** `{original_to}`\n"
        ...
        if parts[0] == "SET":
            ...
            if action == "notif":
                ...
                return await event.edit(
                    f"✅ Deleted {result.modified_count} mails.",
                    buttons=[[Button.inline("⬅️ Back", cb("M", "back"))]]
                )
    ...
    async def on_message(event):
        ...
        if not st:
            if await _is_admin() and text == "🛠️ Admin Panel":
                return await admin_panel(event, bot_instance, col_users, col_aliases, col_logs)
            has_access, _ = await check_user_access(event.sender_id, col_users)
            if has_access:
                quick_actions = {
                    ...
                    "📊 Statistics": lambda: show_user_stats(event, col_users, col_aliases, col_logs, edit=False),
                    "⚙️ Settings":   lambda: show_user_settings(event, col_users, col_logs, edit=False),
        ...
        if st["stage"] == "add_user_wait_id":
            ...
            if not tg_id:
                return await event.reply(
                    "❌ Invalid input.\n\nSend User ID, @username, or forward a message.",
                    buttons=[[Button.inline("❌ Cancel", cb("X", "cancel"))]]
                )
            try:
        ...
        if st["stage"] == "search_user":
            ...
            return await event.reply(
                "❌ User not found.\nTry another ID, username, or name.",
                buttons=[[Button.inline("❌ Cancel", cb("X", "cancel"))]]
            )
        ...
        if st["stage"] == "add_email_pick_user":
            ...
            u = await col_users.find_one({"tg_user_id": target_tg})
            if not u:
                return await event.reply(
                    "❌ User not in database. Add them first.",
                    buttons=[[Button.inline("➕ Add User", cb("U", "add"))], [Button.inline("❌ Cancel", cb("X", "cancel"))]]
                )
...
# =====================================================================
# GENERIC /start, /admin, /help, /inbox, /stats HANDLERS
# =====================================================================
def make_start_handler(bot_instance, col_users, col_aliases, col_logs, super_admin_ids):
    async def on_start(event):
        sender   = event.sender
        username = (sender.username or "") if sender else ""
        name     = (sender.first_name or "") if sender else ""
        await get_or_create_user(event.sender_id, col_users, super_admin_ids, username=username, name=name)
        has_access, status = await check_user_access(event.sender_id, col_users)
        if await is_admin(event.sender_id, col_users, super_admin_ids):
            return await admin_panel(event, bot_instance, col_users, col_aliases, col_logs)
        if not has_access:
            if status == "pending":
        ...
        return await user_panel(event, col_users, col_aliases, col_logs)
    return on_start

def make_admin_cmd_handler(bot_instance, col_users, col_aliases, col_logs, super_admin_ids):
    async def on_admin_cmd(event):
        if not await is_admin(event.sender_id, col_users, super_admin_ids):
            return await event.respond("❌ Admin access required.")
        return await admin_panel(event, bot_instance, col_users, col_aliases, col_logs)
    return on_admin_cmd

def make_inbox_cmd_handler(col_users, col_logs):
...
def make_stats_cmd_handler(col_users, col_aliases, col_logs):
    async def on_stats_cmd(event):
        has_access, _ = await check_user_access(event.sender_id, col_users)
        if not has_access:
            return await event.respond("❌ Access denied.")
        return await show_user_stats(event, col_users, col_aliases, col_logs)
    return on_stats_cmd
...
def register_bot_handlers(
    ...
    bot_instance.add_event_handler(
        make_start_handler(bot_instance, col_users, col_aliases, col_logs, super_admin_ids),
        events.NewMessage(pattern=r"^/start$")
    )
    bot_instance.add_event_handler(
        make_admin_cmd_handler(bot_instance, col_users, col_aliases, col_logs, super_admin_ids),
        events.NewMessage(pattern=r"^/admin$")
    )
    bot_instance.add_event_handler(
        lambda e: show_user_help(e),
        events.NewMessage(pattern=r"^/help$")
    )
    bot_instance.add_event_handler(
        make_inbox_cmd_handler(col_users, col_logs),
        events.NewMessage(pattern=r"^/inbox$")
    )
    bot_instance.add_event_handler(
        make_stats_cmd_handler(col_users, col_aliases, col_logs),
...
async def cache_refresh_loop():
    while True:
        try:
            await refresh_bot1_alias_cache(force=True)
            await refresh_bot2_alias_cache(force=True)
        except Exception as e:
...
async def main():
    logger.info("=" * 60)
    logger.info("  MasterMailBot — Dual Bot + Direct SMTP Receiver")
    logger.info("=" * 60)

    # Start bots
    await bot1.start(bot_token=BOT1_TG_TOKEN)
    await bot2.start(bot_token=BOT2_TG_TOKEN)
    logger.info("✅ Bot1 (Nihal) and Bot2 (Maruf) connected to Telegram")

    # Ensure DB indexes for both databases
    await ensure_indexes(bot1_col_users, bot1_col_aliases, bot1_col_logs)
    await ensure_indexes(bot2_col_users, bot2_col_aliases, bot2_col_logs)
    logger.info("✅ DB indexes ensured for both databases")

    # Initial alias cache load
    await refresh_bot1_alias_cache(force=True)
    await refresh_bot1_alias_tokens(force=True)
    await refresh_bot2_alias_cache(force=True)
    await refresh_bot2_alias_tokens(force=True)
    logger.info("✅ Alias caches loaded")

    # Register handlers for bot1
    register_bot_handlers(
        bot1,
        bot1_col_users, bot1_col_aliases, bot1_col_logs,
        bot1_alias_cache, bot1_alias_token_cache,
        bot1_admin_state, BOT1_SUPER_ADMIN_IDS,
        refresh_bot1_alias_cache, refresh_bot1_alias_tokens,
    )
    # Register handlers for bot2
    register_bot_handlers(
        bot2,
        bot2_col_users, bot2_col_aliases, bot2_col_logs,
        bot2_alias_cache, bot2_alias_token_cache,
        bot2_admin_state, BOT2_SUPER_ADMIN_IDS,
        refresh_bot2_alias_cache, refresh_bot2_alias_tokens,
    )
    logger.info("✅ Event handlers registered for both bots")

    # Start SMTP server
    if AIOSMTPD_AVAILABLE:
        smtp_handler = MasterSMTPHandler()
        smtp_controller = Controller(smtp_handler, hostname=SMTP_HOST, port=SMTP_PORT)
        smtp_controller.start()
        logger.info(f"✅ SMTP server listening on {SMTP_HOST}:{SMTP_PORT}")
    else:
        logger.warning("⚠️  aiosmtpd not installed. SMTP server disabled. Run: pip install aiosmtpd")
        smtp_controller = None

    # Start cache refresh background loop
    refresh_task = asyncio.create_task(cache_refresh_loop())
    logger.info("✅ Background cache refresh started (every 30s)")
    logger.info("🚀 All systems online. Bots are running!")
    ...
    try:
        ...
        mongo1.close()
        mongo2.close()
        logger.info("👋 MasterMailBot shut down cleanly.")
